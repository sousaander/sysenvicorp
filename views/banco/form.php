...
<form action="<?php echo $actionUrl; ?>" method="POST" class="bg-white p-6 rounded-lg shadow-xl max-w-2xl mx-auto">
    ...
    <div class="space-y-6">
        ...
        <!-- Nome -->
        <div>
            <label for="nome" class="block text-sm font-medium text-gray-700 mb-1">Nome do Banco/Conta <span class="text-red-500">*</span></label>
        ...
        <div>
            ...
            <input type="text" id="tipo" name="tipo" placeholder="Ex: Conta Corrente, Poupança, Caixa"
                value="<?php echo $isEdit ? htmlspecialchars($banco['tipo']) : 'Conta Corrente'; ?>"
                class="w-full border-gray-300 rounded-lg shadow-sm focus:border-sky-500 focus:ring-sky-500 p-2">
        </div>

        <!-- Saldo Inicial -->
        <div>
            <label for="saldo_inicial" class="block text-sm font-medium text-gray-700 mb-1">Saldo Inicial (R$) <span class="text-red-500">*</span></label>
    ...
    <div class="mt-8 pt-4 border-t border-gray-200 flex justify-end space-x-4">
        <a href="<?php echo BASE_URL; ?>/banco" class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg shadow-sm hover:bg-gray-200 transition">Cancelar</a>
