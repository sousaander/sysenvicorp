<?php $pageTitle = $pageTitle ?? 'NFS-e - Notas Fiscais de Serviço'; ?>
<div class="p-6 space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">NFS-e</h1>
            <p class="text-sm text-gray-500 mt-1">Notas Fiscais de Serviço Eletrônicas</p>
        </div>
        <?php if (has_permission('fiscal_notas_manage')): ?>
            <a href="<?= BASE_URL ?>/nfse/form" class="px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 text-sm">Nova NFS-e</a>
        <?php endif; ?>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4">
        <form method="GET" class="flex items-end gap-4 flex-wrap">
            ...
            <a href="<?= BASE_URL ?>/nfse" class="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 text-sm">Limpar</a>
        </form>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <table class="w-full text-sm">
            ...
            <tbody>
                <?php if (empty($notas)): ?>
                    <tr><td colspan="6" class="p-8 text-center text-gray-400">Nenhuma NFS-e encontrada.</td></tr>
                <?php else: ?>
                    <?php foreach ($notas as $n): ?>
                        <tr class="border-b border-gray-100 hover:bg-gray-50">
                            <td class="p-3 font-medium text-gray-800"><?= htmlspecialchars($n['numero']) ?></td>
                            <td class="p-3 text-gray-600"><?= htmlspecialchars($n['cliente_nome']) ?></td>
                            <td class="p-3 text-gray-600"><?= date('d/m/Y', strtotime($n['data_emissao'])) ?></td>
                            <td class="p-3">
                            ...
                            <td class="p-3 text-center">
                                <a href="<?= BASE_URL ?>/nfse/detalhe/<?= $n['id'] ?>" class="text-orange-600 hover:text-orange-700 text-xs font-medium">Detalhes</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
